...
def merge_key_candidates(candidate1, candidate2):
    
    merged_candidates = []
    for key1, key2 in zip(candidate1, candidate2):
        if key1 == key2: 
            merged_candidates.append(key1)
        else: 
            merged_candidates.append(-1)
...
def break_6_rounds():
    ...
    possible_keys = differential_attack_6_rounds(cipher, difference, attempts, subkeys)
    round_6_key_candidate = [key[0]["key"] if len(key) > 0 else -1 for key in possible_keys]
    key2= round_6_key_candidate
    print("Key candidate: " + str(round_6_key_candidate))
    key_end = merge_key_candidates(key1,key2)
    print("Final key candidate "+str(key_end))
